...
#define help_string(name,info) printf(name ":\n" info "\n\n");

void print_help(){
  help_string("--version", "Print shoms version string then exit.");
  help_string("--help", "Prints this help message");
  help_string("--affinity", "Uses affinity mode. Launch with 2 jobs and SHOMS will run a selection of tests across different cores between the two nodes. Launching with more than or fewer than 2 jobs will cause the test to abort.");
  help_string("--off_cache", "Works to minimize the effects of CPU caches. Greatly increases memory usage.");
  help_string("--warmup", "Perform warmup runs before doing timed runs to make caches hot.");
  help_string("--msglen FILE_NAME", "Reads file FILE_NAME and uses the sizes listed in the file for test message lengths. Formated with one message size per line.");
  help_string("--minsize NUMBER", "Specifices the minimum size as NUMBER bytes for message lengths when generating message lengths automatically. Ignored if --msglen is passed.");
  help_string("--maxsize NUMBER", "Specifices the maximum size as NUMBER bytes for message lengths when generating message lengths automatically. Ignored if --msglen is passed.");
  help_string("--time NUMBER", "Specifies a soft limit of NUMBER seconds for the maximum amount of time spent on a particular message size for a particular test. This is not precise as a running test won't be interrupted.");
  help_string("--input FILE_NAME", "Specifies an input file called FILE_NAME that defines which tests to run. Tests are named after their OpenSHMEM function. File is formated with one test per line.");
}
